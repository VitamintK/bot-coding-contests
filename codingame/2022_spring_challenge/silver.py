...
homes = [
    [(base_x + 7000*flip, base_y+1400*flip)],
    [(base_x+1400*flip, base_y+7000*flip)],
...
foxholes_2 = [
    ...
    (other_x - 1300 * flip, other_y - 100 * flip),
    (other_x - 100 * flip, other_y - 1300 * flip),
]
    ...
    for i, hero in enumerate(my_heroes):
        ...
        for action in action_scores:
            if action.type != ActionType.MOVE:
                continue
            for monster in monsters:
                if monster.near_base==1 and monster.threat_for==2 and distance_from_position(monster, (action.x, action.y))<=DAMAGE_RANGE:
